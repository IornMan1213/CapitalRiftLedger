...
async function main() {
  ...
  if (snapshot.net_worth == null) {
    console.warn('Warning: net_worth missing — see raw-latest.json');
  }
  const outPath = hourly ? HISTORY_HOURLY : HISTORY_PATH;
  let history = await loadHistory(outPath);
  if (hourly) {
    const key = snapshot.hour_key;
    history = history.filter(r => r.hour_key !== key);
  ...
  console.log('net_worth', snapshot.net_worth, 'cash', snapshot.cash, 'income_24h', snapshot.income_24h);
  console.log('Saved →', outPath, '(' + history.length + ' rows)');
}
